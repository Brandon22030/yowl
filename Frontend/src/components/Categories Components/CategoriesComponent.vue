<template>
  ...
  <h2 class="text-xl font-bold mb-[2rem]">Categories</h2>
    ...
    <RouterLink :to="'/create-category'">  <div class="mb-[1rem] w-full h-[3rem] bg-[#1977a2] items-center flex justify-center text-white font-bold rounded-[10px] cursor-pointer hover:bg-opacity-4">
        ...
        <span @click="categoriesStore.addCategory(categoryName, categoryImage, userId)"> + Add Category +</span>
      ...
      >
        <li @click="categoriesStore.fetchCategory(categorie.id)" >
          ...
          <RouterLink :to="`/categories/${categorie.id}`">
            ...
            <p class="flex items-center p-2 text-gray-900 hover:text-white py-[1rem] dark:text-white hover:bg-[#c38d9e] rounded-[20px] dark:hover:bg-gray-700 group">
              <img src="" alt="" class="rounded-full bg-black w-10 h-10">
              <div > <span class="ms-3">{{ categorie.name }}</span></div>
            </p>
